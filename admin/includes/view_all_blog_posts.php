...
<?php include('includes/blog_info_cards.php'); ?>
<?php
if(isset($_POST['checkBoxArray'])){
    foreach($_POST['checkBoxArray'] as $postValueId){
        ...
        switch($bulk_options){
            case 'approved';
            $query_bulk_approve_post = "UPDATE blog_posts SET post_status = '{$bulk_options}' WHERE post_id = {$postValueId}";
            $run_query_bulk_approve_post = mysqli_query($conn, $query_bulk_approve_post);
            break;

            case 'draft';
            $query_bulk_unapprove_post = "UPDATE blog_posts SET post_status = '{$bulk_options}' WHERE post_id = {$postValueId}";
            $run_query_bulk_unapprove_post = mysqli_query($conn, $query_bulk_unapprove_post);
            break;

            case 'delete';
            $delete_bulk_post_query = "DELETE FROM blog_posts WHERE post_id = '{$postValueId}'";
            $run_delete_bulk_post_query = mysqli_query($conn, $delete_bulk_post_query);
            break;

            case 'clone';
            $edit_posts_query = "SELECT * FROM blog_posts WHERE post_id = '{$postValueId}'";
            ...
            $add_post_query = "INSERT INTO blog_posts (post_category_id, post_author_id, post_title, post_caption, post_image, post_date, post_content, post_status, post_views_count, post_comments_count, post_slug, post_tags) ";
    ...
    <table class="table table__wrapper">
        <div class="row gap-1 mt-2">
            <div id="bulkOptionsContainer" style="width:330px; display:flex; gap:1rem;">
                ...
                <a class="form__button btn btn-primary" href="blog.php?source=add_blog_post">Add New</a>
            </div>
        </div>
        <thead>
            <tr class="table__row">
        ...
        <tbody>
            <?php 
            $query_select_blog_posts = "SELECT * FROM blog_posts ORDER BY post_id DESC";
            $run_query_select_blog_posts = mysqli_query($conn, $query_select_blog_posts);
            if(!$run_query_select_blog_posts) {
                die("Error selecting posts " . mysqli_error($conn));
            }
            while($row = mysqli_fetch_assoc($run_query_select_blog_posts)) {
                $post_id = $row['post_id'];
                $post_category_id = $row['post_category_id'];
                $post_title = $row['post_title'];
                $post_caption = $row['post_caption'];
                ...
                $count_comments_query = "SELECT * FROM blog_comments WHERE comment_post_id = '$post_id'";
            ...
            <tr class="table__row">
                ...
                <?php
                $select_categories = "SELECT * FROM blog_categories WHERE blog_category_id = $post_category_id";
                ...
                ?><a href="blog.php?source=blog_posts_by_category&post_id=<?php echo $post_category_id;?>"><?php echo $cat_title;?></a></td> <?php } ?>
                ...
                <td data-label="Comments" class="table__data"><a href="blog_comments_by_post.php?id=<?php echo $post_id; ?>"><?php echo $comments_count; ?></a></td>
                <td data-label="Approve" class="table__data"><a href="blog.php?approve=<?php echo $post_id; ?>">Approve</a></td>
                <td data-label="Draft" class="table__data"><a href="blog.php?unapprove=<?php echo $post_id; ?>">Draft</a></td>
                <td data-label="Edituj" class="table__data">
                    <a href="blog.php?source=edit_blog_post&p_id=<?php echo $post_id; ?>" class="table__link">
                ...
                <td data-label="Obrisi" class="table__data">
                    <a href="blog.php?delete_blog_post=<?php echo $post_id; ?>" class="table__link">
...
<?php
if(isset($_GET['delete_blog_post'])) {
    $delete_post_id = $_GET['delete_blog_post'];

    $delete_post_query = "DELETE FROM blog_posts WHERE post_id = '{$delete_post_id}'";
    ...
    if(!$run_delete_post_query) {
        ...
        header("Location: blog.php?postDeletedSuccessfully");
    }
}
...
if(isset($_GET['approve'])) {
    ...
    $query_approve_post = "UPDATE blog_posts SET post_status = 'approved' WHERE post_id = {$approve_id}";
    $run_approve_post = mysqli_query($conn, $query_approve_post);

    if(!$run_approve_post) {
        die("Error approving post");
    }else {
        header("Location: blog.php?postApproved");
    }
}
...
if(isset($_GET['unapprove'])) {
    ...
    $query_unapprove_post = "UPDATE blog_posts SET post_status = 'draft' WHERE post_id = {$unapprove_id}";
    $run_query_unapprove_post = mysqli_query($conn, $query_unapprove_post);

    if(!$run_query_unapprove_post) {
        die("Error unapproving post");
    }else {
        header("Location: blog.php?postUnapproved");
    }
}
?>