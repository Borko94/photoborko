<form action="" class="form">
    <div class="form__group">
        <lable class="form__label">Datum</lable>
        <input name="post_image" type="date" class="form__input" placeholder="Foto...">
    </div>
    <div class="row gap-1">
        <div class="form__group col-6-md col-12-xs">
            <label for="" class="form__label">Kategorija</label>
            <select name="" id="" class="form__select">
                <option value="1" class="form__option">Foto</option>
                <option value="1" class="form__option">Video</option>
                <option value="1" class="form__option">Foto + Video</option>
            </select>
        </div>
        <div class="form__group col-6-md col-12-xs">
            <label for="" class="form__label">Musterija</label>
            <select name="" id="" class="form__select">
                <option value="1" class="form__option">Borko</option>
                <option value="1" class="form__option">Stefan</option>
                <option value="1" class="form__option">Vladan</option>
            </select>
        </div>
    </div>
    <div class="row gap-1 ">
        <div class="form__group col-12-xs">
        <label for="" class="form__label">Detalji</label>
        <textarea name="" id="" cols="30" rows="10" class="form__textarea" placeholder="Sadržaj"></textarea>
        </div>
    </div>
    <div class="row gap-1 ">
        <div class="form__group">
            <button class="btn btn-success" type="submit">Dodaj</button>
        </div>
    </div>
</form>