...
<form action="" class="form" method="post" enctype="multipart/form-data">
    <div class="row gap-1">
        <div class="form__group col-4-md col-12-xs">
            ...
            <input name="post_title" class="form__input" type="text" class="form__input" placeholder="Naslov">
        </div>
        
        <div class="form__group col-4-md col-12-xs">
            <label class="form__label">Podnaslov</label>
            <input name="post_caption" class="form__input" type="text" class="form__input" placeholder="Podnaslov">
        </div>
        <div class="form__group col-4-md col-12-xs">
            <label class="form__label">Tags</label>
            <input name="post_tags" class="form__input" type="text" class="form__input" placeholder="Tags">
        </div>
    </div>
    ...
    <div class="row gap-1">
        ...
        <div class="form__group col-6-md col-12-xs">
            ...
            <select name="post_status" id="" class="form__select">
                <option value="draft" class="form__option">Izaberi status</option>
                <option value="approved" class="form__option">Odobren</option>
                <option value="draft" class="form__option">Nacrt</option>
            </select>
        </div>
    </div>
    <div class="row gap-1 ">
    ...
    <div class="row gap-1 ">
        <div class="form__group">
            <button class="btn btn-success" type="submit" name="add_portfolio_post">Submit</button>
        </div>
    </div>
</form>

<?php 
if(isset($_POST['add_portfolio_post'])) {
    ...
    if(in_array($file_actual_ext, $allowed)) {
        if($file_error === 0) {
            ...
            header("Location:posts.php?errorUploadingImage");
        }
    }else {
        header("Location:posts.php?fileTypeNotAllowed");
    }

    

    $add_post_query = "INSERT INTO portfolio_posts (post_category_id, post_author_id, post_title, post_caption, post_image, post_date, post_content, post_status, post_views_count, post_comments_count, post_slug, post_tags, post_edit_date) VALUES ('{$post_category_id}', '{$post_author}', '{$post_title}', '{$post_caption}', '{$file_name_new}', now(), '{$post_content}', '{$post_status}', '{$post_views_count}', '{$post_comment_count}', '{$post_slug}', '{$post_tags}', now() )";
    $run_add_post_query = mysqli_query($conn, $add_post_query);

    if(!$run_add_post_query) {
        die('Error adding new portfolio post' . mysqli_error($conn));
    }else {
        header("Location: portfolio.php?postAddedSuccessfully");
    }
